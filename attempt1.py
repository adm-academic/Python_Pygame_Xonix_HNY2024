...
class DedMoroz(pygame.sprite.Sprite): # Класс спрайта "Дед Мороз"
    ...
    def process_colliions(self): # обрабатываем коллизии деда мороза
        ...
        if ( not self.rect_located_on_border(self.old_rect) ) and \
           ( self.rect_located_on_border(self.rect) ):
            self.process_hide_snow_sprites() # запустим метод гашения участка спрайтов
        # ----- вычислим момент выхода деда мороза со снега на открытую поверхность
        print("&&&&&&&&",len(sprites_matrix), len(sprites_matrix[0]) )
        print("%%%%%%%%",[self.old_rect.y,self.old_rect.x], [self.rect.y,self.rect.x] )
        print("@@@@@@@@",[iYOn,iXOn],[iYNn,iXNn])
        if ( sprites_matrix[iYOn][iXOn] != None ) and \
           ( sprites_matrix[iYNn][iXNn] == None ):
            self.process_hide_snow_sprites() # запустим метод гашения участка спрайтов
        ...
        for hit in hits:
            if (hit.crushed==False): # дед мороз попал на чистый снег
                ...
                crushed_sprites.add(hit) # ...
            else: # дед мороз попал на примятый снег, на свой след
                print("ОПС! СТОЛКНОВЕНИЕ СО СЛЕДОМ! ПЕРЕЗАПУСК УРОВНЯ!")
                exit()

    def clear_colors(self):
        self.logic_colors_counts = {"green": 0, "red": 0}
        for iy in range(1, MATRIX_HEIGHT - 1):
            for ix in range(1, MATRIX_WIDTH - 1):
                if isinstance( sprites_matrix[iy][ix] , Snow ):
                    sprites_matrix[iy][ix].logic_color = ""


    def process_hide_snow_sprites(self): # обрабатываем скрытие зон спрайтов на поле
        begin_cell_1, begin_cell_2 = self.determine_begin_fill_cells()
        print("!!!!!!!!!!!!!!! ", begin_cell_1, begin_cell_2 )
        ...
        print("############", self.logic_colors_counts )
        if self.logic_colors_counts["green"] < self.logic_colors_counts["red"]:
            clear_color = "green"
        else:
            clear_color = "red"
        print("clear_color = ",clear_color)
        for iy in range(1, MATRIX_HEIGHT - 1):
            for ix in range(1, MATRIX_WIDTH - 1):
                if isinstance( sprites_matrix[iy][ix] , Snow ):
                    if ( sprites_matrix[iy][ix].logic_color == clear_color ) or \
                       ( sprites_matrix[iy][ix].crushed == True ):
                        sprites_matrix[iy][ix].kill()
                        sprites_matrix[iy][ix]=None
                    else:
                        sprites_matrix[iy][ix].logic_color = ""
        pass
        self.clear_colors()
    ...
    def floodfill_sprites_zone(self, begin_y , begin_x,  logic_color  ):# возвращает количество заполненых ячеек
        if not isinstance( sprites_matrix[begin_y][begin_x] , Snow ) :
            return
        if sprites_matrix[begin_y][begin_x].crushed == True:
            return
        if sprites_matrix[begin_y][begin_x].logic_color !="":
            return
        sprites_matrix[begin_y][begin_x].logic_color = logic_color
...
for y in range( MATRIX_HEIGHT ):
    ...
    for x in range( MATRIX_WIDTH ):
        sprites_matrix[y].append( pygame.sprite.Sprite() )
# ....................................................................
...
for iy in range(1,MATRIX_HEIGHT-1):
    for ix in range(1,MATRIX_WIDTH-1):
        sp = Snow( SPRITE_SIZE*iy, SPRITE_SIZE*ix )
        sprites_matrix[iy][ix] = sp
        snow_sprites.add(sp)
        all_sprites.add(sp)
# ....................................................................

crushed_sprites = pygame.sprite.Group()
old_sprites     = pygame.sprite.Group()
...
while True:
    ...
    sprite_ded_moroz.keyboard_processing() # обрабатываем ввод для деда мороза
    sprite_ded_moroz.move_auto() # перемещает деда мороза по экрану
